...
if (isset($_GET['id_pedido']) && isset($_GET['id_sala']) && isset($_GET['mesa'])) {
    try {
        ...
        $stmtUpdatePedido = $pdo->prepare("UPDATE pedidos SET estado = 'COMPLETADO' WHERE id = :id_pedido AND estado = 'PENDIENTE'");
        ...
        if ($rowsUpdated > 0) {
            // Otras acciones que puedas necesitar realizar al finalizar el pedido
            ...
            // Si no se actualizó ningún registro, el estado ya estaba actualizado o el pedido no existe
            // Redirigir a la página de mesas con un mensaje de error
